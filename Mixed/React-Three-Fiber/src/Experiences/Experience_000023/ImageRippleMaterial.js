...
export class ImageRippleMaterial extends THREE.ShaderMaterial {
  constructor() {
    super({
      ...
      void main()  {
          ...
          vec3 warpedCol = texture2D( tDiffuse, st ).xyz * 2.0;
          float w = max( warpedCol.r, 0.85);
        	vec2 offset = 0.01 * cos( warpedCol.rg * 3.14159);
          vec3 col = texture2D( tDiffuse, vUv + offset ).rgb * vec3(0.8, 0.8, 1.5) ;
          col *= w*1.2;
          vec4 ripple = vec4( mix(col, texture2D( tDiffuse, vUv + offset ).rgb, 0.5),  1.0);
  ...
  set tDisplacement(v) {
    return (this.uniforms.tDisplacement.value = v)
  }
  get uMouse() {
